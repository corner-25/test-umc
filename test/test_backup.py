...
def create_pivot_table(df):
    ...
    # Thêm tỷ lệ đúng hạn nếu có
    if 'on_time_rate' in pivot_data.columns:
        display_data['on_time_rate_display'] = display_data.apply(
            lambda row: f"{row['on_time_rate']:.1f}%", axis=1
        )
        display_columns.append('on_time_rate_display')
        column_names['on_time_rate_display'] = 'Tỷ lệ đúng hạn (%)'
    
    st.markdown(f"#### 📋 Tổng hợp theo {period_type} (bao gồm biến động)")

    # Hiển thị bảng với cải thiện font size và định dạng
    st.dataframe(
        display_data[display_columns].rename(columns=column_names),
        use_container_width=True,
        height=400  # Tăng chiều cao để dễ đọc
    )

    # Thống kê tóm tắt
    change_display_names = {f'period': f'{period_type}'}
    
    for col in available_columns:
        change_columns.extend([f'{col}_change', f'{col}_change_pct'])
        change_display_names[f'{col}_change'] = f'{col} (+/-)'
        change_display_names[f'{col}_change_pct'] = f'{col} (%)'
    
    # Tạo DataFrame cho hiển thị biến động
    change_data = pivot_data[change_columns].copy()
    ...
    for col in available_columns:
        ...
        if change_col in pivot_data.columns and pct_col in pivot_data.columns:
            # Combine absolute and percentage change
    ...
    for col in available_columns:
        if f'{col}_combined' in change_data.columns:
            final_rename[f'{col}_combined'] = f'{col} (±%)'
    ...
    if len(pivot_data) > 0:
        ...
        with col1:
            total_docs = pivot_data['total_incoming'].sum() if 'total_incoming' in pivot_data.columns else 0
            st.metric("📈 Tổng văn bản", f"{total_docs:,.0f}")
        with col2:
            on_time_docs = pivot_data['processed_on_time'].sum() if 'processed_on_time' in pivot_data.columns else 0
            st.metric("✅ Đúng hạn", f"{on_time_docs:,.0f}")
        with col3:
            late_docs = pivot_data['processed_late'].sum() if 'processed_late' in pivot_data.columns else 0
            st.metric("⚠️ Trễ hạn", f"{late_docs:,.0f}")
        with col4:
            if 'on_time_rate' in pivot_data.columns and len(pivot_data) > 0:
                avg_rate = pivot_data['on_time_rate'].mean()
                st.metric("📊 Tỷ lệ TB", f"{avg_rate:.1f}%")
            else:
                st.metric("📊 Tỷ lệ TB", "0%")
...
def create_incoming_docs_charts(df):
    col1, col2 = st.columns(2)
    
    with col1:
        # Biểu đồ số lượng văn bản theo ngày
        fig_daily = px.line(df, x='datetime', y='total_incoming', 
                           title='📈 Số lượng văn bản đến theo ngày',
                           markers=True)
        fig_daily.update_layout(xaxis_title="Ngày", yaxis_title="Số lượng văn bản")
        st.plotly_chart(fig_daily, use_container_width=True)
        
        # Biểu đồ tỷ lệ xử lý đúng hạn vs trễ hạn
        processed_data = df.groupby('datetime').agg({
            'processed_on_time': 'sum',
            'processed_late': 'sum'
        }).reset_index()
        
        fig_processed = go.Figure()
        fig_processed.add_trace(go.Scatter(x=processed_data['datetime'], 
                                         y=processed_data['processed_on_time'],
                                         mode='lines+markers', name='Đúng hạn',
                                         line=dict(color='green')))
        fig_processed.add_trace(go.Scatter(x=processed_data['datetime'], 
                                         y=processed_data['processed_late'],
                                         mode='lines+markers', name='Trễ hạn',
                                         line=dict(color='red')))
        fig_processed.update_layout(title='⏰ Tình hình xử lý văn bản',
                                  xaxis_title="Ngày", yaxis_title="Số lượng")
    ...
    with col2:
        ...
        # Biểu đồ theo tuần
        weekly_data = df.groupby('week')['total_incoming'].sum()
        fig_weekly = px.bar(x=weekly_data.index, y=weekly_data.values,
                           title='📅 Số lượng văn bản theo tuần')
        fig_weekly.update_layout(xaxis_title="Tuần", yaxis_title="Số lượng văn bản")
        st.plotly_chart(fig_weekly, use_container_width=True)
...
with tab2:
    ...
    if uploaded_file is not None:
        ...
        if df is not None:
            ...
            # Biểu đồ
            create_incoming_docs_charts(df)
